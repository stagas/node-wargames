...
var geodata = geoip.open(path.join(__dirname, '../GeoLiteCity.dat'))

function Wargames(app, options, host) {
	var self = this;
	...
	self.initiateCaching(function () {
		...
		self.server = self.initiateWebsocketServer(host);
	});
};
...
Wargames.prototype.fetchPositions = function () {
	...
	Object.keys(self.users).forEach(function (name) {
        ...
        var info = geoip.City.record_by_addr(geodata, ip);
...
Wargames.prototype.initiateWebsocketServer = function (host) {
	var self = this
    , hserver = http.createServer()

  hserver.listen(5969, host)
